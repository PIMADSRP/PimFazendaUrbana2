...
namespace PIMFazendaUrbanaRadzen.Components.Pages.Insumos
{
    public partial class BaixarInsumo
    {
        ...
        [Inject]
        public InsumosApiService<InsumoDTO> InsumoApiService { get; set; }

        [Inject]
        public InsumosApiService<SaidaInsumoDTO> SaidaInsumoApiService { get; set; }
        ...
        protected async Task FormSubmit()
        {
            try
            {
                ...
                var response = await SaidaInsumoApiService.CadastrarSaidaAsync(saidaInsumo);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    NavigationManager.NavigateTo("/insumos");
                    NotificationService.Notify(NotificationSeverity.Success, "Sucesso", "Saída de insumo cadastrada com sucesso!", duration: 5000);
                }
                else
                {
                    ...
                    NotificationService.Notify(NotificationSeverity.Error, "Erro", $"Falha ao cadastrar saída de insumo: {errorMessage}", duration: 10000);
                }
            }
            catch (Exception ex)
            {
                errorVisible = true; // Exibe mensagem de erro
                Console.WriteLine($"Erro ao cadastrar saída de insumo: {ex.Message}");
            }
        }
